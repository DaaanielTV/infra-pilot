...
class DatabaseManager(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.mysql_root_password = os.getenv(
    ...
    async def create_database_container(
        self, db_name: str, user_id: str, app_id: Optional[str] = None
    ) -> dict:
        ...
        try:
            ...
            container.reload()
            port = container.attrs["NetworkSettings"]["Ports"]["3306/tcp"][0][
                "HostPort"
            ]
    ...
    async def db_create(self, ctx, name: str, app_id: str = None):
        ...
        try:
            ...
            await ctx.send(
                f"\u2705 Database **{name}** created! Check your DMs for credentials."
            )
        except:
            await ctx.send(
                f"\u2705 Database **{name}** created! (Could not DM you the credentials)"
            )
    ...
    async def db_list(self, ctx):
        ...
        for db in databases:
            lines.append(
                f'\u2022 **{db["name"]}** \u2014 Port: {db["port"]} \u2014 Status: {db["status"]}'
            )
        await ctx.send("\n".join(lines))
    ...
    async def db_info(self, ctx, db_id: str):
        ...
        try:
            ...
            await ctx.send(
                "\u274c Could not DM you. Please check your privacy settings."
            )
