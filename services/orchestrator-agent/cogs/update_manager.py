...
class UpdateManager(commands.Cog):
    ...
    @app_commands.command(
        name="updatecheck", description="Check for OS package updates on a VPS"
    )
        ...
        try:
            ...
            if exit_code != 0:
                await interaction.followup.send(
                    embed=discord.Embed(
                        description="Failed to check updates.", color=0xFF0000
                    )
                )
                return
            ...
            embed = discord.Embed(
                title=f"Available Updates: {vps_id[:12]}",
                color=discord.Color.blue(),
                timestamp=datetime.now(),
            )
            embed.add_field(
                name="Updates Available", value=str(len(lines)), inline=True
            )
            ...
            if lines:
                ...
                embed.add_field(
                    name="Packages", value="All packages up to date", inline=False
                )
    ...
    @app_commands.command(
        name="updateapply", description="Apply OS package updates on a VPS"
    )
    @app_commands.describe(vps_id="VPS ID")
    async def update_apply(self, interaction: discord.Interaction, vps_id: str):
        ...
        try:
            ...
            embed = discord.Embed(
                title=f"Updates Applied: {vps_id[:12]}",
