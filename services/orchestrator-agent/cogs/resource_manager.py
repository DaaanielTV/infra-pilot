...
class ResourceManager(commands.Cog):
    ...
    @app_commands.command(
        name="resourcepoolcreate", description="Create a resource pool"
    )
    @app_commands.describe(
        name="Pool name",
        cpu_ratio="CPU oversubscription ratio",
        mem_ratio="Memory oversubscription ratio",
    )
    async def pool_create(
        ...
        if name in self.resource_pools:
            await interaction.response.send_message(
                embed=discord.Embed(description="Pool already exists.", color=0xFF0000)
            )
            return
        ...
        await interaction.response.send_message(
            embed=discord.Embed(
                description=f"Pool '{name}' created (CPU ratio: {cpu_ratio}x, Mem ratio: {mem_ratio}x)",
                color=0x00FF00,
            )
        )

    @app_commands.command(
        name="resourcepooldelete", description="Delete a resource pool"
    )
    ...
    async def pool_delete(self, interaction: discord.Interaction, name: str):
        if name not in self.resource_pools:
            await interaction.response.send_message(
                embed=discord.Embed(description="Pool not found.", color=0xFF0000)
            )
            return
        del self.resource_pools[name]
        await interaction.response.send_message(
            embed=discord.Embed(description=f"Pool '{name}' deleted.", color=0x00FF00)
        )
    ...
    async def pool_list(self, interaction: discord.Interaction):
        if not self.resource_pools:
            await interaction.response.send_message(
                embed=discord.Embed(description="No resource pools.", color=0xFFFF00)
            )
            return
        ...
        for name, pool in self.resource_pools.items():
            total_cpu = sum(
                self.vps_manager.vps_instances.get(m, {})
                .get("config", {})
                .get("cpu_limit", 0)
    ...
    async def pool_add(
        self, interaction: discord.Interaction, pool_name: str, vps_id: str
    ):
        ...
        if vps_id in self.resource_pools[pool_name]["members"]:
            await interaction.response.send_message(
                embed=discord.Embed(description="Already in pool.", color=0xFFFF00)
            )
            return
        self.resource_pools[pool_name]["members"].append(vps_id)
        await interaction.response.send_message(
            embed=discord.Embed(
                description=f"VPS {vps_id[:12]} added to pool '{pool_name}'.",
                color=0x00FF00,
            )
        )
