...
class ModpackInstaller(commands.Cog):
    ...
    async def search_modpacks(
        self, query: str, platform: str = "all", limit: int = 10
    ) -> list:
        results = []
                                        ...
                                        results.append(
                                            {
                                                ...
                                                "iconUrl": mod.get("logo", {}).get(
                                                    "url", ""
                                                ),
                                                "minecraftVersions": (
                                                    [latest.get("gameVersion", "")]
                                                    if latest
                                                    else []
                                                ),
                                                "loaders": (
                                                    latest.get("modLoaders", [])
                                                    if latest
                                                    else []
                                                ),
    ...
    async def install_modpack(
        self, container_id: str, modpack_id: str, platform: str
    ) -> dict:
        try:
            download_url = None
            server_dir = "/data"
                        ...
                        if mod_file.endswith(".jar"):
                            ...
                            subprocess.run(
                                [
                                    ...
                                    "cp",
                                    local_path,
                                    f"{container_id}:{server_dir}/mods/",
                                ],
                                capture_output=True,
                                timeout=60,
                            )
                    ...
                    for config_file in os.listdir(config_dir):
                        ...
                        subprocess.run(
                            [
                                ...
                                "cp",
                                local_path,
                                f"{container_id}:{server_dir}/config/",
                            ],
                            capture_output=True,
                            timeout=60,
                        )
    ...
    async def modpack_search(self, ctx, query: str, platform: str = "all"):
        ...
        embed = discord.Embed(
            title=f'Modpacks: "{query}"', color=discord.Color.purple()
        )
        for r in results:
