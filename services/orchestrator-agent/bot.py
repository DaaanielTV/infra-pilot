    ...
    if conn:
        try:
            ...
            cursor.execute(
                ...
                (container_name[:255], str(userid), container_name[:255], ssh_command or ""),
            )
            conn.commit()
            cursor.close()
            conn.close()
...
def get_container_id_from_database(userid, container_name=None):
    ...
    with open('database.txt', 'r') as f:
        for line in f:
            if line.startswith(str(userid)) and (not container_name or container_name in line):
                return line.split('|')[1]
    return None
...
async def earncredit(interaction: discord.Interaction):
    ...
    try:
        ...
        await interaction.response.send_message("Failed to reach URL shortener service.")
        return
    if response.get('url', {}).get('status') == 7:
        ...
        error_message = response.get('url', {}).get('title', 'Failed to generate a shortened URL.')
...
logging.warning("NOTE: bot.py is a legacy standalone implementation. Use main.py instead for the full feature set.")
bot.run(TOKEN)