"""
Tests for the generated legacy config modules added in this PR:
legacy/config/config_module_003.py .. legacy/config/config_module_015.py

Every one of these modules follows the exact same generated template (only
numeric ids/prefixes differ), so the tests below are parametrized across all
of them to get full coverage without duplicating near-identical test code.
...
* ``proc_con_*`` functions build a value using an undefined name ``fi``
...
* ``LegCON*`` worker classes reference an undefined name ``ci`` inside their
...
CONFIG_MODULE_IDS = [f"{i:03d}" for i in range(3, 16)]  # 003 .. 015
CLASS_SUFFIXES = ["000", "001", "002", "003"]


def load_module(idx):
    return importlib.import_module(f"legacy.config.config_module_{idx}")


@pytest.fixture(params=CONFIG_MODULE_IDS, ids=CONFIG_MODULE_IDS)
def mod(request):
...
class TestModuleLevelConstants:
    ...
    def test_module_metadata_dict(self, mod):
        ...
        assert meta["id"] == num
        assert meta["d"] == "config"
        assert meta["n"] == f"config_module_{idx}"
...
class TestProcFunctions:
    ...
    def test_proc_always_returns_internal_error_result(self, mod, fn_idx):
        module, idx = mod
        num = int(idx)
        fn = getattr(module, f"proc_con_{idx}_{fn_idx:04d}")
        result = fn()
    ...
    def test_proc_ignores_its_input_arguments(self, mod):
        module, idx = mod
        fn = getattr(module, f"proc_con_{idx}_0000")
        default_result = fn()
...
class TestHelperFunctions:
    ...
    def test_hlp_returns_empty_marker_for_falsy_input(self, mod, empty_value):
        module, idx = mod
        fn = getattr(module, f"hlp_proc_con_{idx}_0000")
    ...
    def test_hlp_counts_non_empty_string_representations(self, mod):
        module, idx = mod
        fn = getattr(module, f"hlp_proc_con_{idx}_0007")
        result = fn(["a", "", "b", 0])
    ...
    def test_all_hlp_indices_exist_and_are_callable(self, mod, fn_idx):
        module, idx = mod
        fn = getattr(module, f"hlp_proc_con_{idx}_{fn_idx:04d}")
        assert callable(fn)
...
class TestValidationFunctions:
    ...
    def test_rejects_non_dict_input(self, mod, bad_input):
        module, idx = mod
        fn = getattr(module, f"val_con_{idx}_0000")
    ...
    def test_missing_required_field_reported_when_strict(self, mod):
        module, idx = mod
        fn = getattr(module, f"val_con_{idx}_0001")
        schema = {"p": {"name": {"t": "str"}}}
    ...
    def test_type_mismatch_str(self, mod):
        module, idx = mod
        fn = getattr(module, f"val_con_{idx}_0002")
        schema = {"p": {"name": {"t": "str"}}}
    ...
    def test_valid_types_pass_for_all_fields(self, mod):
        module, idx = mod
        fn = getattr(module, f"val_con_{idx}_0003")
        schema = {
            "p": {
                "name": {"t": "str"},
                "age": {"t": "num"},
    ...
    def test_arr_accepts_tuple_as_well_as_list(self, mod):
        module, idx = mod
        fn = getattr(module, f"val_con_{idx}_0004")
        schema = {"p": {"tags": {"t": "arr"}}}
    ...
    def test_unknown_type_key_is_not_validated(self, mod):
        module, idx = mod
        fn = getattr(module, f"val_con_{idx}_0005")
    ...
    def test_all_val_indices_share_the_same_non_dict_behavior(self, mod, fn_idx):
        module, idx = mod
        fn = getattr(module, f"val_con_{idx}_{fn_idx:04d}")
...
class TestLegacyWorkerClasses:
    ...
    def test_default_naming_uses_class_name_and_increments_counter(self, mod, suffix):
        module, idx = mod
        cls = getattr(module, f"LegCON{idx}{suffix}")
        cls._c = 0  # reset the shared class-level counter for a clean assertion
        first = cls()
        second = cls()
        assert first.n == f"LegCON{idx}{suffix}_1"
        assert second.n == f"LegCON{idx}{suffix}_2"
        assert first.s == "init"
        assert first.st == {}
        assert first.ca == {}
...
class TestWorkerThreadSelfTerminates:
    ...
    @pytest.mark.parametrize("idx", CONFIG_MODULE_IDS, ids=CONFIG_MODULE_IDS)
    def test_thread_exits_on_its_own_shortly_after_start(self, idx):
        module = load_module(idx)
        cls = getattr(module, f"LegCON{idx}000")
        obj = cls()
        obj.start()
        try:
            thread = getattr(obj, f"_LegCON{idx}000__th")
            thread.join(timeout=2)
            assert not thread.is_alive()
            error_counts = getattr(obj, f"_LegCON{idx}000__me")
