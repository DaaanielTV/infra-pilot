...
app = typer.Typer(help="Cellular network management commands")
...
def register(
    ...
    """Register a new cellular network"""
    client = _get_client(ctx)
...
def sims(
    ...
    """List SIM cards for a network"""
    client = _get_client(ctx)
    result = client.cell_sims(network_id)
    data = result if isinstance(result, _list_type) else result.get("sims", result)
    print_output(data, ctx.obj.get("output", "table"))
