...
class Cleanup(commands.Cog):
    ...
    async def _collect_cleanup_data(self, vps_id: str = None) -> dict:
        ...
        try:
            if vps_id:
                ...
                dangling = self.vps_manager.client.images.list(
    ...
    @app_commands.command(
        name="cleanupdryrun", description="Show what would be cleaned up"
    )
    async def cleanup_dry_run(
        self, interaction: discord.Interaction, vps_id: str = None
    ):
        ...
        embed.add_field(
            ...
            value=str(data.get("dangling_images", 0)),
            inline=True,
        )
        embed.add_field(
            name="Total Images", value=str(data.get("docker_images", 0)), inline=True
        )
        ...
        if vps_id:
            embed.add_field(
                name="Container Logs",
                value=str(data.get("container_logs", 0)),
                inline=True,
            )

        embed.add_field(
            name="Would Clean",
            value="Dangling images, unused volumes, package cache",
            inline=False,
        )
    ...
    async def cleanup_run(self, interaction: discord.Interaction, vps_id: str = None):
        ...
        try:
            if vps_id:
                ...
                results.append(
                    f"Package cache: {'cleaned' if exit_code == 0 else 'failed'}"
                )
                ...
                # System-wide cleanup
                deleted_images = self.vps_manager.client.images.prune(
                    filters={"dangling": True}
                )
                results.append(
                    f"Dangling images: {len(deleted_images.get('ImagesDeleted', []))} removed"
                )

                deleted_volumes = self.vps_manager.client.volumes.prune()
                results.append(
                    f"Unused volumes: {len(deleted_volumes.get('VolumesDeleted', []))} removed"
                )
