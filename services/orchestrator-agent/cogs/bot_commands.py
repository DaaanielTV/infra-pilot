...
class BotCommands(commands.Cog):
    ...
    @app_commands.command(
        name="earncredit", description="Earn credits by shortening a URL"
    )
    ...
    @app_commands.command(
        name="renew", description="Renew a VPS for 8 days (costs 2 credits)"
    )
    ...
    async def renew_vps(self, interaction: discord.Interaction, vps_id: str):
        ...
        if credits < 2:
            await interaction.response.send_message(
                embed=discord.Embed(
                    description="You need 2 credits to renew.", color=0xFF0000
                )
            )
            return
        ...
        if not container_id:
            await interaction.response.send_message(
                embed=discord.Embed(
                    description=f"VPS {vps_id} not found.", color=0xFF0000
                )
            )
            return
    ...
    async def deploy_ubuntu(self, interaction: discord.Interaction):
        ...
        if self.vps_manager.count_user_servers(user_id) >= config.SERVER_LIMIT:
            await interaction.followup.send(
                embed=discord.Embed(
                    description="Instance limit reached.", color=0xFF0000
                )
            )
            return
        ...
        try:
            ...
            if ssh_session:
                ...
                await interaction.followup.send(
                    embed=discord.Embed(
                        description="Instance created. Check your DMs for SSH details.",
                        color=0x00FF00,
                    )
                )
            else:
                ...
                await interaction.followup.send(
                    embed=discord.Embed(
                        description="Creation failed. Please try again.", color=0xFF0000
                    )
                )
    ...
    async def _execute_server_action(
        self, interaction: discord.Interaction, container_name: str, action: str
    ):
        user_id = str(interaction.user.id)
        container_id = self.vps_manager.get_container_id_from_database(
        ...
        try:
            ...
            await interaction.response.send_message(
                embed=discord.Embed(
                    description=f"Instance {action}ed successfully.", color=0x00FF00
                )
            )
        except subprocess.CalledProcessError as e:
            await interaction.response.send_message(
    ...
    async def restart_server(
        self, interaction: discord.Interaction, container_name: str
    ):
        ...
        try:
            ...
            if ssh_session:
                try:
                    await interaction.user.send(
                        embed=discord.Embed(
                            description=f"New SSH: ```{ssh_session}```", color=0x00FF00
                        )
                    )
                except Exception:
                    pass
                await interaction.response.send_message(
                    embed=discord.Embed(
                        description="SSH regenerated. Check your DMs.", color=0x00FF00
                    )
                )
            else:
                await interaction.response.send_message(
    ...
    async def remove_server(
        ...
        try:
            ...
            await interaction.followup.send(
                embed=discord.Embed(
                    description="Instance removed successfully.", color=0x00FF00
                )
            )
        except Exception as e:
            await interaction.followup.send(
    ...
    @app_commands.describe(
        container_name="Container name", container_port="Internal port"
    )
    async def port_add(
        self, interaction: discord.Interaction, container_name: str, container_port: int
    ):
        if not self.vps_manager.is_safe_name(container_name):
            await interaction.response.send_message(
                "Invalid container name.", ephemeral=True
            )
            return

        await interaction.response.send_message(
            embed=discord.Embed(
                description="Setting up port forwarding...", color=0x00FF00
            )
        )
        ...
        try:
            ...
            await interaction.followup.send(
                embed=discord.Embed(
                    description="Error setting up port forwarding.", color=0xFF0000
                )
            )

    @app_commands.command(name="port-http", description="Forward HTTP traffic")
    @app_commands.describe(
        container_name="Container name", container_port="Internal HTTP port"
    )
    async def port_http(
        self, interaction: discord.Interaction, container_name: str, container_port: int
    ):
        try:
            ...
            url_line = await self._capture_output(
                exec_cmd, "Forwarding HTTP traffic from"
            )
            if url_line:
                url = url_line.split()[-1]
                await interaction.response.send_message(
                    embed=discord.Embed(
                        description=f"Website available at: {url}", color=0x00FF00
                    )
                )
            else:
                await interaction.response.send_message(
                    embed=discord.Embed(
                        description="Failed to get forwarding URL.", color=0xFF0000
                    )
                )
        except Exception as e:
            await interaction.response.send_message(
