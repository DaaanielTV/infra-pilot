...
class VPSBilling(commands.Cog):
    ...
    async def check_billing_loop(self):
        ...
        try:
            ...
            for container_id, instance in vps_manager.vps_instances.items():
                await self.process_instance_billing(
                    container_id, instance, vps_manager, vps_pricing
                )
        except Exception as e:
            self.logger.error(f"Error in billing loop: {e}")

    async def process_instance_billing(
        self, container_id: str, instance: Dict, vps_manager, vps_pricing
    ):
        """Process billing for a single VPS instance"""
        try:
            last_billing = datetime.fromisoformat(
                instance.get("last_billing", instance["created_at"])
            )
            next_billing = last_billing + timedelta(days=30)
            now = datetime.now()

            if now >= next_billing:
                await self.bill_instance(
                    container_id, instance, vps_manager, vps_pricing
                )

        except Exception as e:
            self.logger.error(
                f"Error processing billing for container {container_id}: {e}"
            )

    async def bill_instance(
        self, container_id: str, instance: Dict, vps_manager, vps_pricing
    ):
        """Bill a VPS instance"""
        ...
        monthly_cost = vps_pricing.calculate_vps_cost(
            config["cpu_limit"], config["memory_limit"], config["storage_limit"]
        )
        ...
        if balance >= monthly_cost:
            ...
            payment_success = await vps_pricing.process_payment(
                user_id, monthly_cost, f"VPS Monthly Billing - {container_id[:12]}"
            )
            ...
            if payment_success:
                ...
                await self.notify_user(
                    ...
                    "VPS Billing Success",
                    {
                        ...
                        "amount": monthly_cost,
                        "next_billing": (datetime.now() + timedelta(days=30)).strftime(
                            "%Y-%m-%d"
                        ),
                    },
                )
            ...
            await self.handle_insufficient_funds(
                container_id, instance, monthly_cost, balance
            )

    async def handle_failed_payment(
        self, container_id: str, instance: Dict, amount: float
    ):
    ...
    async def handle_insufficient_funds(
        self, container_id: str, instance: Dict, amount: float, current_balance: float
    ):
    ...
    async def notify_user(self, user_id: str, notification_type: str, data: Dict):
        ...
        try:
            ...
            if notification_type == "VPS Billing Success":
                embed.description = (
                    "Your VPS monthly payment has been processed successfully."
                )
                ...
                embed.description = (
                    "⚠️ Your VPS payment has failed. Please add funds to your account."
                )
                embed.add_field(name="Container ID", value=data["container_id"])
                embed.add_field(name="Amount Due", value=f"${data['amount']:.2f}")
                embed.add_field(
                    name="Grace Period Ends", value=data["grace_period_end"]
                )
                embed.add_field(
                    name="Warning",
                ...
                embed.description = (
                    "⚠️ You have insufficient funds for your VPS payment."
                )
                embed.add_field(name="Container ID", value=data["container_id"])
                embed.add_field(name="Amount Needed", value=f"${data['amount']:.2f}")
                embed.add_field(
                    name="Current Balance", value=f"${data['current_balance']:.2f}"
                )
                embed.add_field(
                    name="Additional Funds Needed", value=f"${data['needed']:.2f}"
                )
                embed.add_field(
                    name="Grace Period Ends", value=data["grace_period_end"]
                )
                ...
                embed.description = (
                    "❌ Your VPS has been suspended due to payment issues."
                )
                embed.add_field(name="Container ID", value=data["container_id"])
